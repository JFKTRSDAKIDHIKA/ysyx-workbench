...
class UnifiedMemoryController extends Module with RISCVConstants {
  val io = IO(new Bundle {
    // AlignmentNetwork IOs
    val data_in = Input(UInt(32.W))   
    val dmem_addr = Input(UInt(32.W)) 
    val control = Input(UInt(3.W))    
    val data_out = Output(UInt(32.W))
    // MemoryController IOs
...
*/
  ...
  // AlignmentNetwork output assignments
  ...
  // MemoryController output assignments
